# Milestone v0.1.0 — Requirements (ARCHIVED, SHIPPED 2026-06-14)

> Archived at v0.1.0 completion. All in-scope requirements satisfied (STORE·ROUTE·CLIENT·HARDEN·EXAMPLE·RELEASE). EXAMPLE-05, Mode B, Upstash, and the authjs recipe PR were deferred (see the Deferred section + v0.1.0-ROADMAP.md).
...
- [x] **EXAMPLE-01**: Multi-tenant App Router example demonstrates the popup roundtrip end-to-end
- [x] **EXAMPLE-02**: Example deploys to a Vercel preview against a real Microsoft Entra app registration (using the Vercel KV adapter)
- [x] **EXAMPLE-03**: Provider-agnostic proof — bridge mechanics tested in CI against a generic OIDC provider (Keycloak / Auth.js test provider)
- [x] **EXAMPLE-04**: Per-tenant dynamic PWA manifest + `/install-pwa` route present as inert, labeled "Mode B preview" scaffolding. *Constraint: route serves a valid `application/manifest+json` response per request; no Mode B auth flow is wired. Removing this route in v0.1 would break the v0.2 PWA installation entry point.*
...
| EXAMPLE-01 | Phase 5 | Complete |
| EXAMPLE-02 | Phase 5 | Complete |
| EXAMPLE-03 | Phase 5 | Complete |
| EXAMPLE-04 | Phase 5 | Complete |
