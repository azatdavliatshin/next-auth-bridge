...
`THREAT-NN` is the canonical invariant namespace (defined in `.planning/REQUIREMENTS.md`). This file
is the **single source of truth** for those invariants: every row maps a security property to its
mitigation (code evidence, `file:line`) and to a specific, currently-green test (`test-file :: it-name`).
...
| **THREAT-05** | PKCE non-interference: the bridge harvests only session-token chunks, excluding csrf/pkce/state/callback-url decoys, and sets zero cookies on success. | `cookie-codec.ts` harvest filter; `bridge-route.ts` emits zero `Set-Cookie`. | `bridge-route.test.ts :: "harvests only session-token chunks, excluding csrf/pkce/state/callback-url decoys"` |
